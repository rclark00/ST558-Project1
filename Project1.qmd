...
As we saw with `summary()`, calling `plot()` on `example_multi_year` dispatches to `plot.census()` automatically. For this next example we'll plot `year` as the categorical variable and `AGEP` (age) as the numeric variable.
