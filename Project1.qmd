...
summary.census <- function(object, numeric_vars = NULL, categorical_vars = NULL, ...) {
  ...
  if (!all(numeric_vars %in% all_names)) {
    stop("Error: `numeric_vars` contains a variable not found in the data.")
  }
  if (!all(categorical_vars %in% all_names)) {
    stop("Error: `categorical_vars` contains a variable not found in the data.")
  }
...
(placeholder text)... We've seen how `summary()` is generic function above. Another generic function is `plot()`.
